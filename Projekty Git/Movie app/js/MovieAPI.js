class MovieAPI{
    constructor(){
        //this.API_KEY = "";
        this.API_KEY = "f1833fc1204e638ef3c8d49d792f4259";
        this.BASE_URL = "https://api.themoviedb.org/3"
        this. IMAGE_BASE_URL ="https://image.tmdb.org/t/p/w500";
        this.FALLBACK_IMAGE = "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjgwIiBoZWlnaHQ9IjMwMCIgdmlld0JveD0iMCAwIDI4MC"
        this.genres = [];
        this.currentPage = 1;
    ...
    async loadRandomMovies(){
        try {
            ...
            document.getElementById("moviesGrid").innerHTML = `<div class="error" > Failed to load movies.Try again.</div>`;
    ...
    displayMovies(movies, containerId){
        ...
        if(movies.length === 0 ){
            ...
            <div class="no-results">
                ...
                <p> Try adjusting your search criteria or filterss</p>
                </div>
                `;
                return;
        }
    ...
    async handleSearch(query){
        ...
        sectionTitle.textContent = `Search Results for ${trimedQuery}`;
        trendingSection.style.display = "none";

        try {
            document.getElementById("moviesGrid").innerHTML = `<div class="loading"> Searching movies... </div>`;
            let url = `${this.BASE_URL}/search/movie?api_key=${this.API_KEY}&query=${encodeURIComponent(trimedQuery)}&page=1`;
            ...
            document.getElementById("moviesGrid").innerHTML = `div class="error" > Search failed.Try again.</div>`;
    ...
    async handleFilterChange(){
        ...
        await this.handleSearch(searchInput.value.trim())
            ...
            await this.loadFiltersMovies();
        }
    }
     async loadFiltersMovies(){
        
        try {
            document.getElementById("moviesGrid").innerHTML = `<div class="loading"> Loading filtered movies... </div>`;
            let url = `${this.BASE_URL}/discover/movie?api_key=${this.API_KEY}`;
            if(this.currentFilters.genre){
                url += `&with_genres=${this.currentFilters.genre}`;
            }
